...
const playSuggestedSong = asyncHandler(async (req, res) => {
  ...
  const promptToGiveContext = () => {
    ...
    return new Object({
      role: "system",
      content: `# Role: Next Song Recommender # Goal: Based on the currently playing song "${currentSong.name}" by ${currentArtistNames} (genre: 'predict genre[Like Pop, Rock, Jazz, Indie, etc.] based on current song', mood: ${mood}), and (the recent listening history of ${historySongNames} by ${historyArtistNames}) predict at least **20 songs** of different **artists** based on current **genre**,\n Please respond in **exact 1 JSON array** format like:
[
...
Do not include any explanation, just return JSON only.`,
    });
  };
  ...
  for (const { title, artist } of recommendedSongs) {
    try {
